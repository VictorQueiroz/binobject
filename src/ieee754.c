/*
  ...
  Edit by Victor Queiroz to accept 1 byte pointer instead of file stream
*/
...
float read_ieee754_float(uint8_t *input_buffer, int bigendian)
{
   ...
   for(i=0;i<4;i++)
     buff = (buff << 8) | input_buffer[read_offset++];
   if(!bigendian)
   {
     for(i=0;i<4;i++)
     {
...
writedata:
    ...
    if (bigendian)
    {
        buffer[write_offset++] = (hilong >> 24) & 0xFF;
        buffer[write_offset++] = (hilong >> 16) & 0xFF;
        buffer[write_offset++] = (hilong >> 8) & 0xFF;
        buffer[write_offset++] = hilong & 0xFF;

        buffer[write_offset++] = (lowlong >> 24) & 0xFF;
        buffer[write_offset++] = (lowlong >> 16) & 0xFF;
        buffer[write_offset++] = (lowlong >> 8) & 0xFF;
        buffer[write_offset++] = lowlong & 0xFF;
    }
    else
    {
        buffer[write_offset++] = lowlong & 0xFF;
...
writedata:
    ...
    if (bigendian)
    {
        buffer[write_offset++] = (buff >> 24) & 0xFF;
        buffer[write_offset++] = (buff >> 16) & 0xFF;
        buffer[write_offset++] = (buff >> 8) & 0xFF;
        buffer[write_offset++] = buff & 0xFF;
    }
    else
    {
        buffer[write_offset++] = buff & 0xFF;
